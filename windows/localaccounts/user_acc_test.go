...
func (suite *LocalAccTestSuite) TestUser1Read() {
	...
	for _, c := range suite.clients {
		params := localaccounts.UserReadParams{
			Name: "Administrator",
		}
		u, err := c.UserRead(ctx, params)
		suite.Require().NoError(err)
		suite.Equal(localaccounts.User{
...
func (suite *LocalAccTestSuite) TestUser2List() {
	...
	for _, c := range suite.clients {
		...
		suite.Require().NoError(err)
		suite.Contains(u, localaccounts.User{
			AccountExpires:         parsing.DotnetTime{},
			Description:            "Built-in account for administering the computer/domain",
			Enabled:                true,
			FullName:               "",
...
func (suite *LocalAccTestSuite) TestUser3Create() {
	...
	for i, c := range suite.clients {
		params := localaccounts.UserCreateParams{
		...
		suite.NoError(err)
		suite.Equal(localaccounts.User{Name: fmt.Sprintf("Test-User-%d", i)}.Name, g.Name)
		suite.Equal(localaccounts.User{Description: "This is a test user"}.Description, g.Description)
		suite.Equal(localaccounts.User{FullName: fmt.Sprintf("Full-Test-User-%d", i)}.FullName, g.FullName)
		suite.Equal(localaccounts.User{PasswordExpires: parsing.DotnetTime{}}.PasswordExpires, g.PasswordExpires)
		suite.Equal(localaccounts.User{AccountExpires: parsing.DotnetTime(time.Date(2025, time.November, 10, 16, 0, 0, 0, time.UTC))}.AccountExpires, g.AccountExpires)
		suite.Equal(localaccounts.User{UserMayChangePassword: false}.UserMayChangePassword, g.UserMayChangePassword)
		suite.Equal(localaccounts.User{Enabled: true}.Enabled, g.Enabled)
	}
}
...
func (suite *LocalAccTestSuite) TestUser4Update() {
	...
	for i, c := range suite.clients {
		params := localaccounts.UserUpdateParams{
			Name:           fmt.Sprintf("Test-User-%d", i),
...
func (suite *LocalAccTestSuite) TestUser5Delete() {
	...
	for i, c := range suite.clients {
		params := localaccounts.UserDeleteParams{
			Name: fmt.Sprintf("Test-User-%d", i),
		}
